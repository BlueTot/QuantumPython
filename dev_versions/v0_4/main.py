...
class Interpreter:
    ...
    exec_globals = {"qsv": qsv, "measure": measure} # allowed default classes & functions in quantum script
    ...
    @classmethod
    def interpret(cls, code, is_forall=False):
        ...
        while ln < len(code):
            ...
            if new_line == "END": # ending line for custom syntax parsing
                return

            elif re.match(p := r"forall(.*)in(.*):", new_line): # CUSTOM SYNTAX: forall <state> in <qsv>:
                ...
                cls.interpret(c := [l[cls.INDENT_SIZE*indents:] for l in code[ln+1 : end_of_forall_statement]], is_forall=True)
