...
class qstate_inner: # quantum state class, instantiated through the use of the "forall" statement
    ...
    def __and__(self, other): # And method
        ...
        # Checking types
        if not(isinstance(other, qconst) or isinstance(other, qconst_inner) or isinstance(other, qstate)):
            raise TypeError("Argument must be an qconst or qstate")
        ...
        elif isinstance(other, qconst_inner): # argument is a qconst_inner
        ...
        output_reg = QuantumRegister(num_bits) # Create a new output register
        Interpreter.qc.add_register(output_reg) # Add register to quantum circuit
        q1 = Interpreter.qsv_mapping[hash(self.qsv_var)]
        
        for i in range(num_bits): # Loop through all bits
            Interpreter.qc.ccx(q1.qreg[i], q2.qreg[i], output_reg[i]) # AND operation
...
class Interpreter:
    ...
    exec_globals = {"qsv": qsv, "qconst": qconst, "measure": measure, "quantum": quantum} # allowed default classes & functions in quantum script
